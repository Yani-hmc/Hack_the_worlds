...
\begin{itemize}
  ...
  \item \textbf{No VICReg, no SIGReg} needed: collapse resisted by BatchNorm (zero batch variance $\to$ explosion) + EMA lag (non-stationary target)
\end{itemize}
...
% ─── 10. LATENT SPACE VIZ (30s) ─────────────────────────────────────────────
\begin{frame}{Latent Space — What Do the Representations Look Like?}
\begin{columns}[T]
...
\IfFileExists{figures/comparison_4methods_lda_honest.png}{%
  ...
  \begin{center}
  \textcolor{muted}{\footnotesize [Figure pending — job 77882 running]}
  \end{center}
}
\column{0.36\textwidth}
\textbf{Setup:} Conv1D + VICReg $\lambda{=}1$ + corruption (BACC 0.819).
276 eval recordings, mean-pool 16 windows $\to$ 256-dim.
\textcolor{blue!80}{Blue} = Normal, \textcolor{red!80}{red} = Abnormal.

\vspace{0.6em}
\textbf{PCA / UMAP / t-SNE} — classes visually mixed.
\begin{itemize}
  \item VICReg's decorrelation spreads signal across \emph{all 256 dims} equally
  \item 2D projections lose the discriminative directions
  \item Not a contradiction of BACC 0.819
\end{itemize}

\vspace{0.5em}
\textbf{LDA} (4th panel) — fit on \emph{train} set, projected on \emph{eval}.
Finds the best linear direction in 256D that separates the classes.
\begin{itemize}
  \item Out-of-sample: honest, not cherry-picked
  \item Near-clean separation — consistent with 81.9\% BACC
\end{itemize}

\vspace{0.3em}
\textcolor{muted}{\footnotesize Labels never used during pretraining.}
\end{columns}
\end{frame}
