...
\begin{itemize}
  ...
  \item[$\Rightarrow$] Bottleneck is \emph{architecture}, not scale
\end{itemize}
\end{columns}
\end{frame}
...
\vspace{0.5em}
\textbf{Why Conv wins:}\\
{\small No mismatch with corruption augmentation; trains stably; plateaus at 0.82 regardless of depth, width, or data volume.}

\column{0.45\textwidth}
\textbf{EEGPT failure: architectural conflict}
\begin{itemize}
  \item Cross-attention pools 19 ch $\to$ 1 token/step
  \item Channel-drop creates structurally incompatible views before the encoder
...
\begin{frame}{Honest Position vs Literature}
...
\begin{itemize}
  \item \textcolor{good}{$\checkmark$} Matches ContraWR (1.6M) with 0.4M params
  \item \textcolor{warn}{$\times$} 3.9\,pp below LaBraM-Base (5.8M, 200 ep)
  \item Competitive with supervised EEGNet as a \emph{frozen} representation
  \item Per-recording best: 0.825 BACC / 0.913 AUROC
\end{itemize}
\end{columns}
\end{frame}
...
\textbf{NaiveJEPA (TUAB)}   & \textbf{0.389} & \textcolor{good}{probe only} \\
\midrule
...
% ─── 11. LATENT SPACE — BIOT vs EB-JEPA (30s) ───────────────────────────────
\begin{frame}{Latent Space — BIOT Baseline vs EB-JEPA (LDA Comparison)}

\begin{columns}[T]
\column{0.48\textwidth}
\begin{center}
{\small\textbf{EB-JEPA} — VICReg $\lambda{=}1$ + corruption}\\
{\footnotesize BACC\textsubscript{REC} 0.819 (TUAB pretrained, 0.4M params)}
\end{center}
\IfFileExists{figures/lda_honest_tuab.png}{%
  \includegraphics[width=\linewidth]{figures/lda_honest_tuab.png}
}{%
  \begin{center}\textcolor{muted}{\footnotesize [figure pending]}\end{center}
}
...
\IfFileExists{figures/lda_honest_biot.png}{%
  ...
  \begin{center}\textcolor{muted}{\footnotesize [figure pending — BIOT job running]}\end{center}
}
\end{columns}

\vspace{0.2em}
{\small Both LDA: fit on TUAB \emph{train} (2717 rec) → projected on TUAB \emph{eval} (276 rec, honest).
BIOT pretrained on sleep EEG (SHHS) — domain mismatch with clinical pathology may reduce separability.}
\end{frame}
