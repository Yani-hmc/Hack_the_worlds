...
modes --- a VICReg coefficient bug ($\lambda{=}1$ vs.\ paper-correct $\lambda{=}25$),
an architectural conflict between EEGPT's channel-pooling and channel-drop augmentation,
and a per-window vs.\ per-recording evaluation gap (${\approx}5$\,pp) that initially
caused an inflated performance claim.  A new hyperparameter sweep with fixed VICReg
reveals that the spectral auxiliary loss (previously best-in-class at 0.836 per-recording
with the buggy coefficients) provides no independent benefit over correctly-tuned VICReg
alone (0.819), resolving a key open question.
\end{abstract}
...
\caption{%
  ...
  VICReg$^{*}$ = buggy inv\_coeff=1; VICReg = fixed inv\_coeff=25 (paper correct).
  ``--'' = not evaluated.
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Spectral loss with fixed VICReg hurts}: with correct coefficients (inv=25),
    adding spectral reg at 0.05 or 0.10 decreases BACC (0.819 $\to$ 0.802 $\to$ 0.795).
    The previous best of 0.836 (spec=0.1) was achieved with buggy VICReg (inv=1), where
    the spectral loss compensated for the underweighted invariance term.  With correct
    VICReg, spectral regularisation provides no independent benefit.
...
\bottomrule
\multicolumn{7}{l}{$^{\dag}$Trained with buggy VICReg (inv\_coeff=1); result not reproducible with corrected coefficients.}
\end{tabular}}
\end{table}
...
\begin{itemize}[noitemsep]
  ...
  \item The 0.836 REC BACC (spectral + buggy VICReg) is not reproducible with correct
    coefficients (fixed VICReg gives 0.819 without spectral and 0.795 with spec=0.1),
...
\begin{enumerate}[noitemsep]
  \item \textbf{VICReg coefficient bug} ($\lambda{=}1$ vs.\ paper-correct $\lambda{=}25$):
    caused catastrophic collapse for EEGPT and partially degraded all encoders.
    Invariance contribution: $25 \times 0.023 = 0.575$ (fixed) vs.\ $1 \times 0.023 = 0.023$
    (buggy) --- a 25$\times$ underweighting.
  ...
  \item \textbf{Spectral loss independence myth}: the D4 open question is resolved.
    The spectral auxiliary loss (spec=0.1) improved BACC from 0.796 to 0.836 with buggy
    VICReg, but \emph{hurts} (0.819 $\to$ 0.795) with correctly-weighted VICReg.
    The spectral loss was compensating for the invariance underweighting, not providing
    independent signal.
\end{enumerate}
...
\begin{description}[noitemsep]
  ...
  \item[D4] (\emph{Resolved}) Spectral + fixed VICReg: no independent benefit found.
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Loss coefficient bugs are invisible from convergence curves.}
    A 25$\times$ underweighted term may still converge to a ``good enough'' minimum;
    the bug is only detectable by auditing raw loss magnitudes against paper-reported ranges.
